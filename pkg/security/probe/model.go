...
func (p *ProcessEvent) UnmarshalBinary(data []byte) (int, error) {
	...
	if err := binary.Read(bytes.NewBuffer(data[0:16]), byteOrder, &p.CommRaw); err != nil {
		return 0, err
	}
	p.Pid = byteOrder.Uint32(data[16:20])
	p.Tid = byteOrder.Uint32(data[20:24])
	p.UID = byteOrder.Uint32(data[24:28])
	p.GID = byteOrder.Uint32(data[28:32])
